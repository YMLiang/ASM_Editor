...
public class ASMCodeScanner extends RuleBasedScanner {
	private static String[] fgkeywords = {"include","define","start_loop","delay_outer","delay_inner"};
	...
	public ASMCodeScanner(ASMColorProvider provider){
		...
		IToken keyword = new Token(new 
				TextAttribute(provider.getColor(ASMColorProvider.KEYWORD)));
		IToken instruction = new Token(new 
				TextAttribute(provider.getColor(ASMColorProvider.INSTRUCTION)));
		IToken string = new Token(new 
				TextAttribute(provider.getColor(ASMColorProvider.STRING)));
		IToken comment = new Token(new 
				TextAttribute(provider.getColor(ASMColorProvider.SINGLE_LINE_COMMENT)));
		IToken other = new Token(new 
				TextAttribute(provider.getColor(ASMColorProvider.DEFAULT)));
